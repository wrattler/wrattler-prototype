...
// ------------------------------------------------------------------------------------------------
module Wrattler.Gamma.TypeProviders.FSharpProvider
...
let rec unifyTypes ctx schemas tys = 
  ...
  | TypeSchema.Primitive(t1)::ss, t2::ts when AstOps.typesEqual t1 t2 -> unifyTypes ctx ss ts
              ...
              let assigns =
                ...
                |> Seq.map (fun (p, tys) ->
                    p, tys |> Seq.fold (fun st (_, ty2) ->
                      match st with
                      | Some ty1 -> if AstOps.typesEqual ty1 ty2 then Some ty2 else None
  ...
  let objectType = 
    ...
    | typars ->
        { new GenericTypeDefinition with
            ...
            member td.FullName = concatUrl url exp.name
                        ...
                        member x.TypeEquals t2 = 
                          ...
                          | :? GenericType as gt ->
                              gt.TypeDefinition.FullName = x.TypeDefinition.FullName &&
                                AstOps.listsEqual x.TypeArguments gt.TypeArguments AstOps.typesEqual
                          | _ -> false }
