...
// ------------------------------------------------------------------------------------------------
module Wrattler.Gamma.TypeProviders.Pivot
...
module Transform = 
  ...
  let private formatGroupAgg = function
    ...
    | GroupAggregation.CountDistinct(f) -> "count-dist " + AstOps.escapeIdent f
    | GroupAggregation.ConcatValues(f) -> "concat-vals " + AstOps.escapeIdent f
    | GroupAggregation.Sum(f) -> "sum " + AstOps.escapeIdent f
    | GroupAggregation.Mean(f) -> "mean " + AstOps.escapeIdent f
  ...
  let private formatWinAgg = function
    ...
    | WindowAggregation.Mean(f) -> "mean " + AstOps.escapeIdent f
    | WindowAggregation.Min(f) -> "min " + AstOps.escapeIdent f
    | WindowAggregation.Max(f) -> "max " + AstOps.escapeIdent f
    | WindowAggregation.Sum(f) -> "sum " + AstOps.escapeIdent f

  let formatCondition (op, f, v) =
    let op = 
      match op with Equals -> "eq" | NotEquals -> "neq" | LessThan -> "lte" | GreaterThan -> "gte" | InRange -> "in" | Like -> "like"
    AstOps.escapeIdent f + " " + op + " " + AstOps.escapeIdent v
  ...
  let toUrl transforms = 
    [ for t in transforms ->
        ...
        | GroupBy(flds, aggs) -> ["groupby", (List.map (fun fld -> "by " + AstOps.escapeIdent fld) flds) @ (List.map formatGroupAgg aggs)]
        | WindowBy(fld, size, aggs) -> ["windowby", ("by " + AstOps.escapeIdent fld) :: size :: (List.map formatWinAgg aggs)]
        | ExpandBy(fld, aggs) -> ["expandby", ("by " + AstOps.escapeIdent fld) :: (List.map formatWinAgg aggs)]
      ...
      Type.Method
        ( [ for n, t in args -> { MethodArgument.Name = n; Optional = false; Static = false; Type = Type.Primitive t } ],
          (fun ts -> 
              ...
              if AstOps.listsEqual ts args (fun t1 (_, t2) -> AstOps.typesEqual t1 (Type.Primitive t2)) 
