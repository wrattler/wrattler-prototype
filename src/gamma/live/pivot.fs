...
// ------------------------------------------------------------------------------------------------
module Wrattler.Gamma.Live.Pivot
(*
open Fable.Core
open Fable.Import
...
let pickPivotFields expr =
  ...
  | Some { Kind = GammaEntity(GammaEntityKind.MemberAccess { Kind = GammaEntity(GammaEntityKind.Member _); Meta = m }) }
  | Some { Kind = GammaEntity(GammaEntityKind.GlobalValue _); Meta = m }
  | Some { Kind = GammaEntity(GammaEntityKind.Variable(_, { Meta = m })) } -> 
...
let pickPivotTransformations expr =
  let tryPivotType = 
    match expr with
    | { Entity = Some { Type = Some (GammaType(Type.Object (:? TypeProviders.Pivot.PivotObject))) } } -> 
        Some [TypeProviders.Pivot.Transformation.Empty]
    | _ -> None     
  let tryTransform =
    match expr with
    | { Node = Expr.Call({ Entity = Some { Kind = GammaEntity(GammaEntityKind.MemberAccess({ Kind = GammaEntity(GammaEntityKind.Member _); Meta = m })) } }, _) } 
    | { Entity = Some { Kind = GammaEntity(GammaEntityKind.MemberAccess { Kind = GammaEntity(GammaEntityKind.Member _); Meta = m }) } } -> 
...
let findPreview (ch:NestedChain) trigger globals (ent:Entity) = 
  let msg = 
    match ent.Kind with 
    | GammaEntity(GammaEntityKind.MemberAccess({Kind = GammaEntity(GammaEntityKind.Member(_, {Kind=GammaEntity(GammaEntityKind.MemberName n)}))})) -> n
    | GammaEntity(GammaEntityKind.Call _) -> "call"
    | _ -> "whatever"
  ...
  let prevDom = 
    ...
    |> Seq.tryPick (fun (loc, node) ->
      ...
      | Some(GammaType(Type.Object(FindMember nm m))) ->
