...
module TypeChecker = 
  ...
  let numericOperatorMismatch op typ1 typ2 rng =
    { Number = 302; Range = rng 
      Message = 
        sprintf "Both operands of binary operator '%s' should be the same, but the left was %s and right was %s." 
          (formatToken (TokenKind.Operator op)) (formatTypeInfo typ1) (formatTypeInfo typ2) }
